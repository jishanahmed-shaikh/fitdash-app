...
import { Search, Calendar, User, Clock, ChevronRight, PlusCircle, Trash2 } from "lucide-react";
...
// Extended blog data with more content
const blogPosts = [
  {
    id: 1,
...
const initialUserBlogs = JSON.parse(localStorage.getItem('userBlogs') || '[]');

const BlogPage = () => {
  const [searchTerm, setSearchTerm] = useState("");
  const [activeCategory, setActiveCategory] = useState("all");
  const [allPosts, setAllPosts] = useState([...blogPosts, ...initialUserBlogs]);
  const [newBlogTitle, setNewBlogTitle] = useState("");
  const [newBlogExcerpt, setNewBlogExcerpt] = useState("");
  const [newBlogContent, setNewBlogContent] = useState("");
  const [newBlogCategory, setNewBlogCategory] = useState("Personal");
  ...
  const handleAddBlog = () => {
    ...
    const newBlog = {
      ...
      readTime: `${Math.max(1, Math.ceil(newBlogContent.length / 1000))} min read`,
      image: "https://images.unsplash.com/photo-1455390582262-044cdead277a?ixlib=rb-1.2.1&auto=format&fit=crop&w=500&q=60",
      tags: [newBlogCategory.toLowerCase(), "personal"],
      isUserCreated: true // Flag to identify user-created blogs
    };

    const updatedUserBlogs = [...initialUserBlogs, newBlog];
    localStorage.setItem('userBlogs', JSON.stringify(updatedUserBlogs));
    
    setAllPosts(prev => [...prev, newBlog]);
    ...
    setNewBlogExcerpt("");
    setNewBlogContent("");
    setNewBlogCategory("Personal");
    setIsDialogOpen(false);
  };

  const handleDeleteBlog = () => {
    if (blogToDelete) {
      // Filter out the deleted blog from state
      const updatedPosts = allPosts.filter(post => post.id !== blogToDelete);
      setAllPosts(updatedPosts);
      
      // Update localStorage to remove the deleted blog
      const updatedUserBlogs = initialUserBlogs.filter((blog: any) => blog.id !== blogToDelete);
      localStorage.setItem('userBlogs', JSON.stringify(updatedUserBlogs));
      
      toast({
        title: "Blog Deleted",
        description: "Your blog post has been successfully removed",
      });
  ...
  const categories = ["all", ...new Set(allPosts.map(post => post.category.toLowerCase()))];

  const isUserCreatedBlog = (postId: number) => {
    return allPosts.find(post => post.id === postId && post.isUserCreated);
  };
          ...
          <div className="max-w-6xl mx-auto space-y-8">
            <div className="flex justify-between items-center">
              <div className="space-y-2">
                <h1 className="text-3xl md:text-4xl font-bold tracking-tight">Fitness Blog</h1>
                <p className="text-xl text-muted-foreground">
                  Latest insights, tips, and stories to help you on your fitness journey.
                </p>
              </div>
              <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
                ...
                <DialogContent className="sm:max-w-[625px]">
                  <DialogHeader>
                    ...
                    <DialogDescription>
                      Share your fitness journey, tips, or insights with the community.
                    </DialogDescription>
                  </DialogHeader>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 animate-fade-in">
              {filteredPosts.length > 0 ? (
                filteredPosts.map((post) => (
                  <Card key={post.id} className="overflow-hidden hover-scale relative">
                    {isUserCreatedBlog(post.id) && (
                      <AlertDialog>
                        <AlertDialogTrigger asChild>
                          <Button 
                            variant="destructive" 
                            size="icon" 
                            className="absolute top-2 right-2 z-10 h-8 w-8 bg-red-500/80 hover:bg-red-600"
                            onClick={() => setBlogToDelete(post.id)}
                          >
                    ...
                    <CardContent className="pt-6">
                      <div className="flex items-center mb-2">
                        ...
                        {post.isUserCreated && (
                          <span className="ml-2 text-xs font-medium px-2.5 py-0.5 rounded bg-blue-100 text-blue-700 dark:bg-blue-900 dark:text-blue-300">
                            Personal
                          </span>
                        )}
                      </div>
                    ...
                    <CardFooter className="pt-0">
                      <Button asChild variant="ghost" className="p-0 h-auto text-fitdash-purple hover:text-fitdash-purple-dark hover:bg-transparent">
                        <Link to={`/blog/${post.id}`} className="flex items-center text-sm font-medium">
